...
const uint32_t onBoard_switch = (1<<9);

void bus_off_cb(uint32_t temp)
{

    printf("In bus off callback!!\n");

}

void data_ovr_cb(uint32_t temp)
{
    printf("In data ovr callback!!\n");
    CAN_reset_bus(can1);
}

/*
 * CAN subscribe msg test
 */
canSubscribedMsg::canSubscribedMsg(uint16_t periodMs, uint8_t priority) : scheduler_task("canTask1", 2 * 512, priority),
...
bool canSubscribedMsg::init(void)
{

//    LOG_INFO("%d ms TASK STARTED\n", mTaskPeriodMs);
//    printf("Delete '%s' : %s\n","log.csv", (FR_OK == f_unlink("log.csv")) ? "OK" : "ERROR");

    LPC_PINCON->PINSEL2 &= ~(3<<18);
    LPC_GPIO1->FIODIR &= ~(onBoard_switch); //set P1.9 as input

    bool state = CAN_init(can1, 100, 16, 16, bus_off_cb, data_ovr_cb); //Init the can bus
                                                                        /* Can1
                                                                         * 100kbps
                                                                         * tx_queue size = 16
                                                                         * rx_queue size = 16                                                                         *
    ...
    CAN_reset_bus(can1);           //Enable CAN bus

    return true;
}

bool canSubscribedMsg::run(void *p)
{
//    static int count = 0;
//
//    if(count++ <=1000)
//    {
//        LOG_INFO("In %d ms task\n", mTaskPeriodMs);
//    }

    bool bus_state = CAN_is_bus_off(can1);
    if(bus_state)
    {
        printf("CAN BUS OFF\n");
        CAN_reset_bus(can1);           //Enable CAN bus
    ...
    msg.frame_fields.data_len = 1;       // Send 1 byte

    if(!(LPC_GPIO1->FIOPIN & onBoard_switch))
    {
        msg.msg_id = 0x123;
        msg.data.bytes[0] = 0x11; // Write all 8 bytes of data at once
        printf("%x\n", msg.data.bytes[0]);
    ...
    {
        msg.msg_id = 0x124;
        msg.data.bytes[0] = 0x22; // Write all 8 bytes of data at once
        printf("%x\n", msg.data.bytes[0]);
        CAN_tx(can1, &msg, portMAX_DELAY);
    }
