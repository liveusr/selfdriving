...
typedef enum {
    ...
    MSG_DIST_SENSOR_DATA    = 0x0C, // DATA: dist_sensor_data_t, SENSOR to ALL, MASTER/IO/BT will hook, periodic
    MSG_OTHER_SENSOR_DATA   = 0x11, // DATA: other_sensor_data_t, SENSOR to ALL, IO/BT will hook, periodic

    MSG_GEO_HEADING_DATA    = 0x0E, // DATA: geo_heading_data_t, GEO to ALL, MASTER/IO/BT will hook, periodic
    MSG_GEO_LOCATION_DATA   = 0x12, // DATA: geo_location_data_t, GEO to ALL, IO/BT will hook, periodic

    MSG_SPEED_ENCODER_DATA  = 0x0D, // DATA: speed_encoder_data_t, MOTOR to ALL, MASTER/IO/BT will hook, periodic
    ...
    MSG_CHECKPOINT_REQUEST  = 0x0A, // DATA: checkpoint_request_data_t, GEO to BT, when current checkpoint expires
    MSG_CHECKPOINT_DATA     = 0x10, // DATA: checkpoint_data_t, BT to GEO, when MSG_CHECKPOINT_REQUEST received, IO will hook to display progress bar, periodic

    MSG_DRIVE_MODE          = 0x0B, // DATA: drive_mode_data_t, IO/BT to ALL, MASTER/IO/BT/MOTOR will hook
    MSG_SPEED_DIR_COMMAND   = 0x0F, // DATA: speed_dir_data_t, MASTER to MOTOR, periodic
    MSG_FREE_RUN_DIR        = 0x09, // DATA: speed_dir_data_t, BT to MASTER, valid only in free run
...
typedef struct {
        double   latitude;                  // convert convert GPS/MAP received string to float
        double   longitude;                 // convert convert GPS/MAP received string to float
...
typedef struct {
        ...
        uint8_t  dir;                       // of type motor_direction_t
        uint8_t  is_moving;                 // of type motor_speed_t
...
typedef struct {
        double   latitude;                  // convert convert convert GPS/MAP received string to float
        double   longitude;                 // convert convert convert GPS/MAP received string to float
