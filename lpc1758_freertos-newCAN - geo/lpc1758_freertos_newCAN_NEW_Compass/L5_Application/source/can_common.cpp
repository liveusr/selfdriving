...
void can_controller :: enum_to_string(void)
{
    ...
    strcpy(message_id_str[MSG_GEO_HEADING_DATA],        "MSG_GEO_HEADING_DATA");
    strcpy(message_id_str[MSG_GEO_LOCATION_DATA],       "MSG_GEO_LOCATION_DATA");
...
//                msg_id.raw = msg.msg_id;
//                if(*msg_num != (msg_id_t) msg_id.msg_num) {
//                    SET_ERROR(ERROR_MSG_CORRUPT);
//                    LOG_ERROR("msg corrupt\n");
//                    printf("[0x%02X] msg corrupt %s-%s\n", error_flag, message_id_str[*msg_num], message_id_str[msg_id.msg_num]);
//                }
            }
        }
    }
