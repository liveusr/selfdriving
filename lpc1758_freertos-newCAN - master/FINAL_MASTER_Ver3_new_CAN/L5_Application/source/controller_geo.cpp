...
geo_data_t periodic_geo_data = {10.2f, 12.4f, 36, 8, 6, 4, 1};
...
void controllerInit(void)
{
    ...
    scheduler_add_task(new periodicGeoTask(PRIORITY_HIGH));

    /* setup debug variables */
    scheduler_task *dt = new debugTask(PRIORITY_MEDIUM);
    scheduler_add_task(dt);
    ((debugTask *)dt)->setLeftRight(&periodic_geo_data.desired_angle, &periodic_geo_data.current_angle, 0);
    ((debugTask *)dt)->setHex(&error_flag, 1);
}
...
bool canRxBufferTask :: handle_heartbeat(msg_t msg)
{
